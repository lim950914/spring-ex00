...
<nav class="navbar navbar-expand-lg navbar navbar-dark bg-primary">
  ...
  <form action="${listUrl }" class="form-inline">
	<select name="type" class="form-control mr-sm-2">
		<option value="">--</option> 
		<option value="T" ${cri.type == "T" ? 'selected' : '' }>제목</option> <%-- ${cri.type == "T" ? 'selected' : '' }  --%>
		<option value="C" ${cri.type == "C" ? 'selected' : '' }>내용</option> <%-- 3항 연산식 true면 (selected) --%>
		<option value="W" ${cri.type == "W" ? 'selected' : '' }>작성자</option> <%--false이면 ('') --%>
